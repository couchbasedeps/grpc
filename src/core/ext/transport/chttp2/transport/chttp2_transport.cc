...
void grpc_chttp2_cancel_stream(
  ...
  // Destroy the metadata before we potentially destroy the stream
...
static void close_from_api(
  ...
  GRPC_DCHECK(
      ...
      send_trailing_metadata->get_pointer(grpc_core::GrpcMessageMetadata()) ==
          nullptr ||
      send_trailing_metadata->get_pointer(grpc_core::GrpcMessageMetadata())
  ...
  // Capture stream properties before marking it closed.
